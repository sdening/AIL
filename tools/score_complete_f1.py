...
def score_prompt(intro: str, legal_clause, pos_examples: list[str], neg_examples: list[str], modified_prompt: str, initial_prompt: str, df: pd.DataFrame, cat:str, groq_client, weight: dict[float] = {'p':1.0, 'r':0, 'l':0.0}) -> int:

    #readability_rater = sr.TextUnderstandabilityRater()
    scorer= sp.PerformanceScorer(df, cat, intro, modified_prompt, groq_client, legal_clause, neg_examples, pos_examples, verbose=True)
